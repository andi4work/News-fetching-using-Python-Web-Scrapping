...
def fetchNews(page, category):
    ...
    if content['ResultData'][0]['Value'] is None:
        return 0
    else:
...
for index, category in enumerate(categories):
    cat_id = categories[index]
    status = False
    page = 0
    print('## Looking page: ' + str(page) + ' category: ' + category)
    while status == False:
        ...
        for news in list:
            ...
            if dublicate_control(news['Id']) == 0:
                ...
                val = (cat_id, news['HeadLine'].replace('(*#*)', '/'), news['CreateTime'], news_content,
